...
describe('Test Data Generator', () => {
    ...
    describe('generateTestWaveform', () => {
        ...
        it('should generate multiple GPU-safe LOD levels', () => {
            ...
            // At least one LOD, but the exact count depends on track length vs GPU limits
            expect(waveform.lods.length).toBeGreaterThanOrEqual(1);

            for (const lod of waveform.lods) {
                // Each LOD must be well-formed
                expect(lod.samplesPerPixel).toBeGreaterThan(0);
                expect(lod.lengthInPixels).toBeGreaterThan(0);

                // Critical invariant from the generator: each LOD must fit in a 1×N texture
                // under the assumed max texture width of 8192.
        ...
        it('should handle short duration tracks', () => {
            ...
            // Short tracks should still get the full LOD ladder (they all fit under 8192 px)
